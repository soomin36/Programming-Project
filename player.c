...
void Batters(Batter batters[]) {
	...
	strcpy(batters[16].name, "전준우"); batters[14].batting = 83; 
	strcpy(batters[17].name, "고승민"); batters[14].batting = 73;
	strcpy(batters[18].name, "문보경"); batters[14].batting = 81; 
	strcpy(batters[19].name, "문현빈"); batters[14].batting = 87; 
	strcpy(batters[20].name, "노시환"); batters[14].batting = 76; 
}

void Pitchers(Pitcher pitchers[]) {
	strcpy(pitchers[0].name, "안우진"); pitchers[0].command = 90; pitchers[0].speed = 97;
	strcpy(pitchers[0].name, "원태인"); pitchers[0].command = 90; pitchers[0].speed = 87;
	strcpy(pitchers[0].name, "고영표"); pitchers[0].command = 88; pitchers[0].speed = 76;
	strcpy(pitchers[0].name, "김광현"); pitchers[0].command = 85; pitchers[0].speed = 87;
	strcpy(pitchers[0].name, "양현종"); pitchers[0].command = 85; pitchers[0].speed = 81;
	strcpy(pitchers[0].name, "임찬규"); pitchers[0].command = 85; pitchers[0].speed = 80;
	strcpy(pitchers[0].name, "박세웅"); pitchers[0].command = 82; pitchers[0].speed = 85;
	strcpy(pitchers[0].name, "곽빈"); pitchers[0].command = 80; pitchers[0].speed = 97;
	strcpy(pitchers[0].name, "구창모"); pitchers[0].command = 88; pitchers[0].speed = 82;
	strcpy(pitchers[0].name, "류현진"); pitchers[0].command = 88; pitchers[0].speed = 83;
}
